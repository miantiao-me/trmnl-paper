...
3. 选择组件路径（**优先使用高阶组件 / Components**）：
    - 第一优先：**Rich Text / Item / Table / Chart / Progress**。能直接映射时，不要先拆成零散 `div`、`grid`、`flex`
    - 文案、引言、通知、段落 → `richtext`（旧 `markdown` 路线已废弃）
    - 列表、日程、事件 → `item`
    - 简单表格、结构化数据 → `table`
    - 数据可视化 → `chart` 原生 class / Highcharts / Chartkick
    - 进度表达 → `<x-trmnl::progress>` 或 `div.progress-dots`
    - KPI、大数字 → `value` + `label`
    - 多区域 / 多列 → `columns` / `mashup` / `grid`
    - 颜色 token、工具类、`data-*` 属性 → 原生 class / data 属性
    - 具体 props、限制与 gotchas 按需读 `references/components/`、`references/attributes.md`、`references/patterns.md`
4. 从基础骨架开始，再填充模块。
...
1. `references/design-rules.md`：先确认结构层级与 e-paper 约束
2. `references/patterns.md`：先找最接近的布局模板，不要从零拼装
3. `references/components/`：按实际组件查 props、CSS 映射与 gotchas
4. `references/attributes.md`：需要工具类、`data-*` 属性时再读
5. `references/examples/_index.md`：需要完整官方示例时先看索引，再按需展开
6. `references/colors.md` / `references/guides.md`：需要颜色 token 或 v3 差异时再查

### 按需速查

- 组件速查（props / CSS 映射 / 用法）：读 `references/components/` 目录下对应组件文件
- 工具类与 data 属性（间距、尺寸、圆角、可见性等）：读 `references/attributes.md`
- `utilities.md` 兼容入口：读 `references/utilities.md`（权威内容已收敛到 `attributes.md`）
...
收到 `trmnl-paper-screen` 回传的问题清单时，字段格式以 `skills/trmnl-paper-screen/SKILL.md` 的“交接格式”为准：
